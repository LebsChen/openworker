    ...
    if (!p.needs_key)
      return (
        <span className="block text-[11.5px] text-faint truncate">
          {keylessOk.has(p.name) ? <span className="text-ok font-medium">{translate("settings.providers.running")}</span> : "No key needed"}
        </span>
      );
    return <span className="block text-[11.5px] text-faint truncate">{translate("settings.providers.notSetUp")}</span>;
  };
      ...
      <div className="flex gap-2">
        <div className="relative flex-1 min-w-0">
          ...
          {ps.savedState && testable && (
            ...
            >
              {info?.needs_key ? <>{translate("settings.providers.testedSaved")}</> : <>{translate("settings.providers.detected")}</>}
            </span>
          )}
        </div>
        {testable && (
    ...
    <div>
          ...
          <div className="mt-2.5 rounded-xl border border-line bg-paper/60 px-4 pb-3.5 pt-3">
            ...
            {selected?.command && (
              <button
                ...
                title={translate("settings.providers.copyCommand")}
            ...
            <div className="mt-3.5 flex items-center justify-between gap-3 border-t border-line pt-3">
              {ps.savedState ? (
                ...
                <span className="text-[11.5px] text-faint">{translate("settings.providers.runsOneReadOnlyCheckThenSaves")}</span>
              )}
              <button
                ...
                data-testid={`${tp}-test`}
              >
                {ps.verify.state === "testing" ? "…" : <>{translate("settings.providers.testSave")}</>}
              </button>
            </div>
          </div>
        </div>
      ...
      {info?.name === "openai" && (
        <div className="mt-5 rounded-xl border border-line bg-paper/50 px-3.5 pb-3.5 pt-2.5">
          <div className="text-[12px] font-semibold text-ink">{translate("settings.providers.extraRequestHeaders")}</div>
          ...
          <div className="mt-2 space-y-1.5">
            {headerRows.map((row, index) => (
              <div className="flex gap-1.5" key={`${row.name}-${index}`}>
                <input
                  className={input + " border-line"}
                  placeholder={translate("settings.providers.headerName")}
                  value={row.name}
          ...
          <div className="mt-2 flex items-center gap-2">
            <button className="text-[12px] text-muted hover:text-ink" onClick={() => setHeaderRows((rows) => [...rows, { name: "", value: "" }])}>{translate("settings.providers.addHeader")}</button>
            <button
      ...
      {info?.name === "openai" && (
        <div className="mt-5 rounded-xl border border-line bg-paper/50 px-3.5 pb-3.5 pt-2.5">
          <div className="text-[12px] font-semibold text-ink">{translate("settings.providers.discoverEndpointModels")}</div>
          <p className="mt-1 text-[11.5px] leading-relaxed text-faint">

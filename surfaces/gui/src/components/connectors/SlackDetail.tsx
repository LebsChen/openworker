  ...
  return (
    <div data-testid="slack-workspaces">
      <div className="flex items-center gap-3.5 mb-5">
        <ConnectorBadge connector={c} size={44} title={translate("connectors.slack.slack")} />
        <div className="min-w-0 flex-1">
          ...
          <div className="text-[12.5px] text-muted flex items-center gap-1.5">
            {c.connected ? (
              ...
              <span>{translate("connectors.slack.notConnected")}</span>
            )}
          </div>
        </div>
        {relay || !c.connected ? (
      ...
      {c.connected && !relay && (
        <div data-testid="slack-manual-card">
          <div className={GRP_H}>{c.account || "workspace"} <span className="font-normal text-faint">{translate("connectors.slack.manualTokens")}</span></div>
          <div className={GRP}>
            <PeopleRow
      ...
      {c.connected && (
        <div className={FOOT + " mt-2"}>{translate("connectors.slack.namesFromSlack")}</div>
      )}
      ...
      {adding && (
        <AddConnectionModal
          c={c}
          cloud={cloud}
          title={translate("connectors.slack.addWorkspace")}
          ...
          <>
            <div className={ROW}>
              <span className="min-w-0 flex-1 text-[12.5px] text-muted flex items-center gap-2 flex-wrap">
                <span>{translate("connectors.slack.noOneAllowedYetMentionsBotShowUpHereOk")}</span>
...
function DisconnectBtn({ teamId, busy, onClick }: { teamId: string; busy: boolean; onClick: () => void }) {
  return (
    <button
      ...
      title={translate("connectors.slack.stopRelay")}
      onClick={onClick}
      disabled={busy}
    >
  ...
  return (
    <div className={ROW}>
      ...
      <span className="min-w-0 flex-1 flex flex-wrap items-center gap-1.5">
        {allowed.length === 0 && (
          <span className="text-[12px] text-faint">{translate("connectors.slack.approveOrNameSender")}</span>
        )}
        {allowed.map((u) => (
          <span
            key={u}
          ...
          >
            ...
            {label(u)}
            {u === installerId && <span className="text-[10.5px] text-faint">{translate("connectors.slack.you")}</span>}
            {protectedIds?.includes(u) ? (
              <span
                className="text-[10.5px] text-faint"
                title={translate("connectors.slack.removeOwnerFirst")}
              >
                · owner
              </span>
            ) : (
              <button className={XBTN} title={translate("connectors.slack.remove")} onClick={() => onRemove(u)}>
                ×
              </button>
            )}
          </span>
  ...
  return (
    <span className="relative" ref={wrap}>
      <button
        ...
        title={translate("connectors.slack.pickWorkspaceDirectory")}
        onClick={toggle}
      >
        {buttonLabel}
      </button>
      {open && (
        ...
        >
          <input
            ...
            placeholder={translate("connectors.slack.typeName")}
            value={q}
          ...
          <div className="max-h-56 overflow-y-auto py-1">
            {err ? (
              ...
              <div className="px-2 py-1.5 text-[12px] text-faint">{translate("connectors.slack.noMatches")}</div>
            ) : (
              candidates.map((m) => (
                <button
                  key={m.id}
      ...
      <span className="min-w-0 flex-1 flex flex-wrap items-center gap-1.5">
        ...
        {owners.map((u) => (
          ...
          >
            ...
            {label(u)}
            {u === installerId && <span className="text-[10.5px] text-faint">{translate("connectors.slack.installer")}</span>}
            {editable && (
              <button className={XBTN} title={translate("connectors.slack.removeApprovalOwner")} onClick={() => remove(u)}>
                ×
              </button>
            )}
          </span>
        ...
        {!editable && owners.length > 0 && (
          <span className="text-[11.5px] text-faint">{translate("connectors.slack.setWorkspaceInstaller")}</span>
        )}
  ...
  return (
    <div className={ROW + " bg-warnSoft/25"} data-testid={`waiting-${m.id}`}>
      ...
      <button
        ...
        title={translate("connectors.slack.allowSenderDeliverMessageNow")}
      ...
      <button
        ...
        title={translate("connectors.slack.allowSenderMessageDiscarded")}
        onClick={() => act("allow")}
      >
        Allow
      </button>
      <button className={XBTN + " px-1"} data-testid={`parked-dismiss-${m.id}`} title={translate("connectors.slack.dismiss")} onClick={() => act("dismiss")}>
        ×
      </button>
    </div>
  );
        ...
        {subs.map((s) => (
          <span key={s.session_id + s.channel} className="flex items-center gap-2 text-[12.5px]">
            ...
            <button
              ...
              title={translate("connectors.slack.unsubscribeSession")}
              onClick={async () => {

  ...
  return (
    <div data-testid="hubspot-detail">
      <div className="flex items-center gap-3.5 mb-5">
        <ConnectorBadge connector={c} size={44} title={translate("connectors.hubspot.hubspot")} />
        <div className="min-w-0 flex-1">
          ...
          <div className="text-[12.5px] text-muted flex items-center gap-1.5">
            {c.connected ? (
              ...
              <span>{translate("connectors.hubspot.notConnected")}</span>
            )}
          </div>
        </div>
      ...
      {portals.length > 0 && (
        <>
          <div className={GRP_H + " !mt-0"}>{translate("connectors.hubspot.portals")}</div>
      ...
      {adding && (
        <AddConnectionModal
          c={c}
          cloud={cloud}
          title={translate("connectors.hubspot.addPortal")}
  ...
  return (
    <div className={ROW} data-testid={`hubspot-portal-${p.hub_id}`}>
      <span className="min-w-0 flex-1 flex items-center gap-2">
        ...
        {p.default && <span className={TAG_ACCENT}>{translate("connectors.hubspot.default")}</span>}
        {p.sandbox && <span className={TAG_WARN}>{translate("connectors.hubspot.sandbox")}</span>}
        {p.access && (
          <span className={TAG_QUIET} data-testid={`hubspot-access-tag-${p.hub_id}`}>
            {p.access === "write" ? "read & write" : "read-only"}
          </span>
        )}
        {!p.managed && <span className={TAG_QUIET}>{translate("connectors.hubspot.privateApp")}</span>}
      </span>
      {!p.default && (
        <button
      ...
      <button
        className={XBTN}
        title={translate("connectors.hubspot.disconnectPortal")}
...
function PrivacyGroup({ c, onChanged }: Pick<DetailProps, "c" | "onChanged">) {
  ...
  return (
    <>
      <div className={GRP_H}>{translate("connectors.hubspot.accessPrivacy")}</div>
      <div className={GRP}>
        <div className={ROW} data-testid="hubspot-hidden-fields">
          <span className={LABEL}>{translate("connectors.hubspot.hiddenFields")}</span>
          <span className="min-w-0 flex-1 flex flex-wrap items-center gap-1.5">
            {fields.map((f) => (
              ...
              >
                {f}
                <button className={XBTN} title={translate("connectors.hubspot.remove")} onClick={() => save(fields.filter((x) => x !== f))}>
                  ×
                </button>
              </span>
            ))}
            <input
              ...
              placeholder={translate("connectors.hubspot.propertyNameEGSalary")}
              value={draft}
      ...
      <div className={FOOT}>{translate("connectors.hubspot.stripAllRecords")}</div>
    </>
  );
}

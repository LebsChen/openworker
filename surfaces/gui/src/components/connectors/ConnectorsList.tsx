  ...
  return (
    <div>
      <div className="flex items-center justify-end mb-4">
        <input
          placeholder={translate("connectors.list.search")}
          value={filter}
      ...
      <div className={GRP_H}>{translate("connectors.list.available")}</div>
      <div className={GRP}>
        {shown.map((c) => (
        ...
        {shown.length === 0 && (
          <div className={ROW + " text-[12.5px] text-muted"}>{translate("connectors.list.nothingMatches")}</div>
        )}
      </div>
...
function healthChip(c: Connector, slack: SlackStatus | null) {
  ...
  if (c.name === "slack" && c.mode === "relay" && slack) {
    if (!slack.signed_in) return <span className={CHIP_WARN}>{translate("connectors.list.signNeeded")}</span>;
    if (slack.relay.state === "offline") return <span className={CHIP_OFF}>{translate("connectors.list.offline")}</span>;
    if (slack.relay.state === "reconnecting")
      return <span className={CHIP_WARN}>{translate("connectors.list.reconnecting")}</span>;
    if (Object.values(slack.teams).some((t) => !t.token_ok))
      return <span className={CHIP_WARN}>{translate("connectors.list.token")}</span>;
    return <span className={CHIP_OK}>{translate("connectors.list.live")}</span>;
  }
  if (c.two_way && c.connected) return <span className={CHIP_OK}>{translate("connectors.list.live")}</span>;
  return <span className={CHIP_OK}>{translate("connectors.list.ready")}</span>;
}

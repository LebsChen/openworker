...
export function GalleryModal({
          ...
          return (
            ...
            >
              ...
              <div className="shrink-0 flex items-center">
                {isInstalled ? (
                  <span className="text-[12px] text-muted">{translate("gallery.installed")}</span>
                ) : (
                  <span className="text-[12.5px] text-accent">{translate("gallery.viewInstall")}</span>
                )}
              </div>
            </div>
          );
  ...
  const detailView = detailSlug && (
    <div data-testid="gallery-detail">
      ...
      {!detail ? (
        <div className="text-[12.5px] text-muted">{translate("gallery.loading")}</div>
      ) : !detail.ok || !card ? (
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="shrink-0">
              {installed.has(detailSlug) ? (
                <span className="text-[12.5px] text-muted">{translate("gallery.installed")}</span>
              ) : (
          ...
          {caps && (
            <div className={CARD + " p-4"} data-testid="gallery-capabilities">
              ...
              <div className="space-y-2 text-[12.5px]">
                <div>
                  <span className="text-muted">{translate("gallery.tools")}</span>
                ...
                <div>
                  <span className="text-muted">{translate("gallery.permissions")}</span>
                ...
                {(detail.recommends?.length ?? 0) > 0 && (
                  <div>
                    <div className="text-muted mb-1.5">{translate("gallery.worksTheseConnections")}</div>
    ...
    <div className="fixed inset-0 z-50" data-testid="gallery-modal">
      ...
      <div className="absolute left-1/2 top-[6vh] -translate-x-1/2 w-[720px] max-w-[94vw] max-h-[88vh] rounded-xl2 border border-line bg-panel shadow-2xl overflow-hidden flex flex-col">
        <div className="px-5 pt-4 pb-3 border-b border-line flex items-center gap-3 shrink-0">
          <div className="min-w-0 flex-1">
            <div className="text-[15px] font-semibold">{translate("gallery.personaGallery")}</div>
          ...
          {cloud?.signed_in && !detailSlug && (
            <input
              ...
              placeholder={translate("gallery.searchPersonas")}
          ...
          <button
            ...
            onClick={onClose}
            aria-label={translate("gallery.closeGallery")}
        ...
        <div className="overflow-y-auto hairline-scroll p-5">
          {loading ? (
            <div className="space-y-2" data-testid="gallery-loading" aria-busy="true">
              <div className="text-[12.5px] text-muted mb-3">{translate("gallery.loadingGallery")}</div>
              {[0, 1, 2].map((i) => (
            ...
            <div className={CARD + " p-5 flex items-center gap-4"} data-testid="gallery-signin">
              <div className="min-w-0 flex-1">
                <div className="font-semibold text-[14px] mb-1">{translate("gallery.signBrowseGallery")}</div>

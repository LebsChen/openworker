      ...
      <div className="flex-1 min-w-0 overflow-y-auto hairline-scroll">
        <div className="max-w-3xl mx-auto px-7 py-6">
          {tab === "remote" ? (
            ...
            <section>
              <PanelHead
                title={translate("settings.models")}
...
function RemoteHostsSection() {
  ...
  return (
    <section>
      <PanelHead
        title={translate("settings.rvmRemoteVirtualMachines")}
      ...
      <div className={`${CARD} p-4 space-y-3`}>
        ...
        <input className={INPUT} placeholder={translate("settings.eGLinuxDevServer")} value={name} onChange={(e) => setName(e.target.value)} />
        ...
        <input className={INPUT} type="password" placeholder={translate("settings.tokenShownWhenAgentJsStarts")} value={token} onChange={(e) => setToken(e.target.value)} />
        <input className={INPUT} type="password" placeholder={translate("settings.leaveEmptyReuseToken")} value={vncPassword} onChange={(e) => setVncPassword(e.target.value)} />
        <button
          className={BTN_BORDERED}
        ...
        {saved && <div className="text-[12px] text-accent">{translate("settings.remoteHostsSaved")}</div>}
...
function VoiceInputSection() {
  ...
  return (
    <section>
      <PanelHead
        title={translate("settings.voiceInput")}
        sub="Speak naturally in the composer. Recordings and transcripts stay on this device."
      />

      {!desktop ? (
        <div className={CARD + " p-4 text-[13px] text-muted"}>{translate("settings.voiceSetupAvailable")}</div>
      ) : (
        <div className="space-y-4">
          <div className="rounded-xl border border-green-200 bg-green-50/70 px-4 py-3 text-[12.5px] text-green-800">
            <span className="font-medium">{translate("settings.privateDesign")}</span> Audio is held in memory only while you record and is transcribed locally.
          </div>
          ...
          <div className={CARD}>
            <div className="p-4 flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1">
                <div className="text-[13.5px] font-medium">{translate("settings.device")}</div>
            ...
            <div className="border-t border-line bg-paper/50 px-4 py-3 grid grid-cols-2 gap-3 text-[12px] text-muted">
              <div><span className="block text-ink font-medium">Mac</span>{translate("settings.macos12AppleSiliconM1")}</div>
              <div><span className="block text-ink font-medium">Windows</span>{translate("settings.windows1022H211X64")}</div>
              <div><span className="block text-ink font-medium">Memory</span>{translate("settings.8GbRecommended")}</div>
              <div><span className="block text-ink font-medium">Processor</span>{translate("settings.4CpuCoresRecommended")}</div>
            </div>
          </div>
          ...
          <div className={CARD}>
            <div className="p-4 flex items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                <div className="text-[13.5px] font-medium">{translate("settings.whisperBaseEnglish")}</div>
              ...
              {status?.model_verified ? (
                <>
                  ...
                  <button className="text-[12px] text-red-600 px-2 py-2" onClick={() => void remove()}>{translate("settings.delete")}</button>
                </>
              ) : downloading ? (
                <button className={BTN_BORDERED} onClick={() => void cancelDownload()}>{translate("settings.cancel")}</button>
              ) : phase === "verifying" ? (
                <span className="text-[12px] text-muted">{translate("settings.verifying")}</span>
              ) : (
                <button className={BTN_ACCENT} disabled={!status?.supported} onClick={() => void download()}>{translate("settings.downloadModel")}</button>
              )}
            </div>
            {downloading && (
          ...
          <div className={CARD}>
            <div className="p-4 flex items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                <div className="text-[13.5px] font-medium">{translate("settings.microphoneTest")}</div>
                <div className="text-[12px] text-muted mt-0.5">
                  {ready ? "Your microphone and local transcription engine are working." : "Record a short phrase to enable the composer microphone."}
                </div>
              </div>
              {ready && <span className="text-[11.5px] px-2 py-1 rounded-full bg-green-50 text-green-700">{translate("settings.ready")}</span>}
            ...
            {status?.recording && <div className="border-t border-line px-4 py-3 text-[12px] text-accent" role="status">{translate("settings.listeningSpeakShortPhraseThenStop")}</div>}
...
function PersonasSection({ onOpenPersona }: { onOpenPersona?: (id: string) => void }) {
  ...
  return (
    <section>
      <PanelHead
        title={translate("settings.personas")}
      ...
      >
        ...
        <span className="min-w-0 flex-1">
          <span className="block text-[13.5px] font-medium">{translate("settings.browsePersonaGallery")}</span>
          <span className="block text-[12px] text-muted">
            Curated coworkers from the OpenWorker team — see what each can do before installing.
          </span>
        </span>
        <span className="text-[12.5px] text-accent shrink-0">{translate("settings.open")}</span>
      </button>
      {galleryOpen && (
        <GalleryModal
...
function AppearanceSection() {
  ...
  return (
    <section>
      <PanelHead title={translate("settings.general")} sub="How OpenWorker looks and behaves on this machine." />

      <div className={CARD + " p-4 mb-4"}>
        <div className={FIELD_LABEL}>Theme</div>
        <div className="seg mt-2.5" role="radiogroup" aria-label={translate("settings.appearance")}>
        ...
        <div className={FIELD_HELP}>{translate("settings.autoFollowsMacAppearance")}</div>
      </div>
      ...
      {desktop && (
        <div className={CARD + " p-4"}>
          <div className={FIELD_LABEL + " mb-2.5"}>{translate("settings.always")}</div>
          <label className="flex items-start gap-3 py-2">
            <input type="checkbox" className="mt-0.5" checked={autostart} onChange={(e) => toggleAuto(e.target.checked)} />
            <span>
              <span className="block text-[13px] text-ink">{translate("settings.openLogin")}</span>
              <span className="block text-[12px] text-muted">{translate("settings.launchOnSignIn")}</span>
            </span>
          </label>
          <label className="flex items-start gap-3 py-2">
            <input type="checkbox" className="mt-0.5" checked={keepAwake} onChange={(e) => toggleKeep(e.target.checked)} />
            <span>
              <span className="block text-[13px] text-ink">{translate("settings.keepSystemAwake")}</span>
              <span className="block text-[12px] text-muted">{translate("settings.preventIdleSleep")}</span>
            </span>
          </label>
        </div>
      )}
      ...
      <div className={CARD + " p-4 mt-4"}>
        <div className={FIELD_LABEL + " mb-2"}>{translate("settings.setupUpdates")}</div>
        ...
        <div className={FIELD_HELP}>{translate("settings.replayFirstRunSetup")}</div>
      </div>
    </section>
  );
}
...
function TrustedWorkspacesCard() {
  ...
  return (
    <div className={CARD + " p-4 mb-4"} data-testid="trusted-workspaces-card">
      <div className={FIELD_LABEL}>{translate("settings.trustedWorkspaces")}</div>
      <div className={FIELD_HELP}>
        Trusted projects may manage their command allowances in .coworker/config.toml.
      </div>
      {workspaces === null ? (
        <div className="text-[12px] text-muted mt-3">{translate("settings.loading")}</div>
      ) : workspaces.length === 0 ? (
        <div className="text-[12px] text-muted mt-3">{translate("settings.noWorkspacesTrusted")}</div>
      ) : (
...
function TokenSavingsCard() {
  ...
  return (
    <div className={CARD + " p-4 mb-4"} data-testid="token-savings-card">
      <div className={FIELD_LABEL}>{translate("settings.tokenSavings")}</div>
      <div className={FIELD_HELP}>
        PDF attachments travel with every turn of a conversation, so large documents multiply
        what you spend on tokens.
      </div>

      <div className="mt-3 text-[13px] text-ink">{translate("settings.pdfsModelsWithoutNativePdfSupport")}</div>
      <div className="seg mt-2" role="radiogroup" aria-label={translate("settings.pdfFallback")} data-testid="pdf-fallback">
        <button
      ...
      <div className="mt-3 flex items-center gap-5">
        <label className="flex items-center gap-2.5">
          <span className="text-[13px] text-ink">{translate("settings.maxPages")}</span>
          <input
            type="number"
            min={1}
            max={100}
        ...
        <label className="flex items-center gap-2.5">
          <span className="text-[13px] text-ink">{translate("settings.maxSize")}</span>
          <input
            type="number"
            min={1}
            max={10}
          ...
          />
          <span className="text-[12.5px] text-muted">{translate("settings.mb")}</span>
        </label>
      </div>
      <div className={FIELD_HELP}>
...
function CompactionCard() {
  ...
  return (
    <div className={CARD + " p-4 mb-4"} data-testid="compaction-card">
      <div className={FIELD_LABEL}>{translate("settings.contextCompaction")}</div>
      <div className={FIELD_HELP}>
      ...
      <div className="mt-3 flex items-center gap-5 flex-wrap">
        <label className="flex items-center gap-2.5">
          <span className="text-[13px] text-ink">{translate("settings.compact")}</span>
          <input
            type="number"
            min={10}
            max={95}
          ...
          />
          <span className="text-[12.5px] text-muted">{translate("settings.contextWindow")}</span>
        </label>
        <label className="flex items-center gap-2.5">
          <span className="text-[13px] text-ink">{translate("settings.label")}</span>
          <input
            type="number"
            min={10_000}
            max={2_000_000}
          ...
          />
          <span className="text-[12.5px] text-muted">{translate("settings.tokensWhicheverSmaller")}</span>
        </label>
      </div>
      <div className={FIELD_HELP}>
      ...
      <div className="mt-3 flex items-center gap-2.5">
        <span className="text-[13px] text-ink">{translate("settings.summarizerModel")}</span>
        <select
        ...
        >
          <option value="">{translate("settings.sessionOwnModelDefault")}</option>
          {models.map((m) => (
  ...
  return (
    <div className={CARD + " p-4 mb-4"}>
      ...
      <label className="flex items-center gap-3 mt-2.5">
        <span className="text-[13px] text-ink">{translate("settings.conversationsShownPerCoworker")}</span>
        <input
          type="number"
          min={1}
          max={50}
...
function FilesCard() {
  ...
  return (
    <div className={CARD + " p-4 mb-4"}>
      <div className={FIELD_LABEL}>{translate("settings.files")}</div>
        <div className="flex items-center gap-2 mt-2.5">
          <input
            className={INPUT}
            type="text"
            placeholder={translate("settings.openworker")}
            value={scratchDraft}
            spellCheck={false}
            autoComplete="off"
          ...
          />
          {desktop && (
            <button className={BTN_BORDERED} onClick={browseScratch} title={translate("settings.pickFolder")}>
              Browse
            </button>
          )}
